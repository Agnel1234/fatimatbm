    ...
    public partial class FamilyPopup : Form
    {
        ...
        public bool LoadFamilyInfoById(int familyID)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                familyZonetxt.Text = row[3] != null ? row[3].ToString() : null;
                multipleCardsCheckbox.Checked = row[12] != DBNull.Value && Convert.ToBoolean(row[12]);
                familySubscriptiontxt.Text = row[12] != null ? row[12].ToString() : null;
                permAddresstxt.Text = row[4] != null ? row[4].ToString() : null;
                permCitytxt.Text = row[6] != null ? row[6].ToString() : null;
                permStatetxt.Text = row[7] != null ? row[7].ToString() : null;
                permZipcodetxt.Text = row[8] != null ? row[8].ToString() : null;
                tempAddresstxt.Text = row[5] != null ? row[5].ToString() : null;
                tempCitytxt.Text = row[9] != null ? row[9].ToString() : null;
                tempStatetxt.Text = row[10] != null ? row[10].ToString() : null;
                tempZipcodetxt.Text = row[11] != null ? row[11].ToString() : null;
                multipleCardsCheckbox.Checked = row[13] != DBNull.Value && Convert.ToBoolean(row[13]);
                txtFamilyNotes.Text = row[14] != null ? row[14].ToString() : null;
                dtLastSubscription.Value = row[15] != DBNull.Value ? Convert.ToDateTime(row[15]) : DateTime.Now;
